    ...
    for a in articlesByCcs[ccs]:
        ...
        for kw in a['authorTags']:
            if model.has_index_for(kw):
                overlap.append(cosine(model.get_vector(kw), abstract_vec))
        if (len(overlap) != 0):
...
with open("QuickData/vec2wordoverlapping1", "wb") as f:
    pickle.dump(result, f)
#por cada ccs, lista de los overlappings de sus articulos
with open("QuickData/vec2wordoverlapping1raw", "wb") as f:
